...
public final class PaperTickList<T> extends ServerTickList<T> { // extend to avoid breaking ABI
    ...
    public PaperTickList(final ServerWorld world, final Predicate<T> excludeFromScheduling, final Function<T, ResourceLocation> getMinecraftKeyFrom,
                         final Consumer<NextTickListEntry<T>> tickFunction) {
        super(world, excludeFromScheduling, getMinecraftKeyFrom, tickFunction);
        this.world = world;
    ...
    public void nextTick() {
        ...
        if (this.currentTick != this.world.getGameTime()) {
            if (!this.warnedAboutDesync) {
                ...
                MinecraftServer.LOGGER.error("World tick desync detected! Expected " + this.currentTick + " ticks, but got " + this.world.getGameTime() + " ticks for world '" + this.world.dimension().location() + "'", new Throwable());
    ...
    public void copy(MutableBoundingBox structureboundingbox, BlockPos blockposition) {
        ...
        while (iterator.hasNext()) {
            ...
            if (structureboundingbox.isInside( nextticklistentry.getPosition())) {
                BlockPos blockposition1 = nextticklistentry.getPosition().offset(blockposition);
