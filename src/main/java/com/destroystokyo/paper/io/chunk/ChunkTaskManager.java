...
public final class ChunkTaskManager {
    ...
    public static void dumpAllChunkLoadInfo() {
        ...
        if (chunks.length > 0) {
            ...
            for (final ChunkInfo chunkInfo : chunks) {
                ...
                PaperFileIOThread.LOGGER.log(Level.ERROR, chunkInfo.chunkX + "," + chunkInfo.chunkZ + " in '" + chunkInfo.world.dimension().location().toString() + ":");
            ...
            if (!chunkHolder.neighbors.isEmpty()) {
                ...
                for (ChunkHolder neighbor : chunkHolder.neighbors.keySet()) {
                    ...
                    if (seenChunks.contains(neighbor)) {
                        PaperFileIOThread.LOGGER.log(Level.ERROR, indentStr + "  " + nx + "," + nz + " in " + chunkHolder.getWorld().dimension().location() + " (CIRCULAR)");
                        continue;
                    }
                    PaperFileIOThread.LOGGER.log(Level.ERROR, indentStr + "  " + nx + "," + nz + " in " + chunkHolder.getWorld().dimension().location() + ":");
    ...
    public void flush() {
        ...
        if (this.workers == null) {
            if (GoldenForge.isPrimaryThread() || MinecraftServer.getServer().isStopped()) {
                ((ThreadTaskExecutor<Runnable>)this.world.getChunkSource().mainThreadProcessor).runAllTasks();
            } else {
